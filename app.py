...
def build_bom():
            ...
            rows.append(
                {
                    ...
                    "Part Code": (
                        part
                        if part.strip() and part.lower() != "nan"
                        else ""
                    ),
                    "Description": r["Description"],
                    "Quantity": qty,
    ...
    for _, r in pdus_df.iterrows():
        ...
        if qty > 0:
            cost = r["Unit Cost"]
            desc = (
                f'{r["Description"]} | Type: {r["Type"]} | '
                f'C13: {r["C13"]} | C19: {r["C19"]}'
            )
...
def cost_summary(bom):
    ...
    base_cost = (
        float(cfg["Base Cost"])
        if cfg is not None and pd.notna(cfg["Base Cost"])
        else 0.0
    )
...
def add_selling_prices(
    ...
    known_cost_total = (
        result["Total Cost"].fillna(0).sum()
        if not result.empty
        else 0
    )
...
if not bom.empty:
    ...
    internal_cost_data = [
        ...
        [
            "Final Selling Price",
            final_selling_price if is_internal else 0,
        ],
